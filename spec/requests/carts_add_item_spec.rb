...
describe 'POST /cart/add_item', type: :request do
  ...
  context 'when the cart exists in the session' do
    before do
  ...
  context 'when the cart is missing from the session' do
    it 'returns a cart not found error' do
      post '/cart/add_item', params: { product_id: product1.id, quantity: 1 }
      expect(response).to have_http_status(404)
      json = response.parsed_body
